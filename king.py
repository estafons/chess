#king
def move_king(curr_pos,chessboard,next_pos):
   flag=False
   if (abs(int(curr_pos[1])-int(next_pos[1]))==abs(ord(curr_pos[0])-ord(next_pos[0])))and(abs(int(curr_pos[1])-int(next_pos[1]))==1): #move is valid (diagonal)
	   chessboard, flag = c.checkinbetween_diagonal(curr_pos,chessboard,next_pos)
   elif (curr_pos[0]==next_pos[0])and(abs(int(curr_pos[1])-int(next_pos[1]))==1):
       chessboard, flag=  c.checkinbetweenhorizontal(curr_pos,chessboard,next_pos)
   elif (curr_pos[1]==next_pos[1])and(abs(ord(curr_pos[0])-ord(next_pos[0]))==1):
